...
自顶向下不断二分链表，然后自底向上不断合并链表（合并的过程完全是合并两个排序链表的过程）。

时间复杂度：O(n log n)

空间复杂度：O(log n)，二分链表过程中需要的栈空间

解法2：（优化空间复杂度）

解法1 的空间复杂度来源于自顶向下的二分过程中的栈空间，因此想办法优化这一部分。

解决办法是显式的指定归并的子链表的长度：第一次循环，把所有长度为 1 的子链表两两归并；第二次循环，所有长度为 2 的子链表两两归并；
第三次循环，长度为 4...；...

（代码写下来很像 链表交换节点位置 的升级版本，中间有很多需要判断是否为空的部分，**需要慎重加慎重！**）
...
使用数字指代各个节点的状态：0 - 未被访问过；1 - 正在访问 or 有环；2 - 确定是安全节点

接下来对各个节点做 DFS，如果路径上遇到状态为 1 的节点，则返回 False 向上层传递信息，使上层节点也为 1。

时间复杂度：O(n + m)，n 为图上节点个数，m 为图上的边的个数。**原因：图上的边最多被遍历一次。**

空间复杂度：O(n)，递归栈空间 and 状态数组

**方法2：反图 + 拓扑排序**

第一次做拓扑排序相关的题。拓扑排序是什么？

（P.S.：入度 - 进入该顶点的边数；出度 - 从该顶点出发的边数）

拓扑排序：从每个图的入度为 0 的顶点开始，这些顶点指向的顶点的入度 -1 （相当于从图中抹去入度为 0 的顶点），在这个过程中有些顶点的
入度变为 0，然后以这些节点为顶点，重复上述过程。

在**有向无环图**中，必定能够遍历所有节点。

如果存在环，就无法全部遍历。举个例子：

假设一张图中：`A -> B`，`B C D` 三个顶点成环， `D -> E`。

那么拓扑排序得到的结果只包含 A，也就是所有入度为 0 或者 排序过程中更新后的入度为 0 的点。

但本题恰好相反。以上述的图为例，本题在上述图中的最终结果是节点 E。

也就是说，本题是要得到所有出度为 0 或者 排序过程中更新后的出度为 0 的点。

因此我们计算给定的 `graph` 的**反图**（将所有边反向）。反图中的环依然和原图中的环是相同节点。

对反图计算拓扑排序，所有更新后的原图上出度为 0 的节点就是最终结果。

时间复杂度：O(n + m)

空间复杂度：O(n + m)，比方法1多出来的 O(m) 是反图所需的空间
...
使用一个哈希表 match 存储位置 i 的可选数字，使用一个全局的集合 vis 记录已选择的数字即可。

时间复杂度：O(n!)，参考全排列复杂度 n x n!，由于这里不需要记录每一个排列的内容，因此回溯只需要 n! 复杂度

空间复杂度：O(n ^ 2)

方法2：状态压缩 + DP

DP 数组 f 表示当前状态下的优美排列的个数。例如，`f["100110"]` 表示排列的前三位选择 2,3,6 时的优美排列个数
（不考虑其他位，只考虑前三位的情况）

以 100110 为例，就是选择 2,3,6 作为优美排列的前三项。当我们计算 `f["100110"]` 的时候，我们考虑 2,3,6 这三个数哪个能放在排列的第 3 位。
3 可以，因此把 3 放在第 3 位的优美排列数量等于把 2,6 放在前两位的优美排列数量，因此 `f["100110"] += f["100010"]`。

同理 6 也可以，因此把 6 放在第 3 位的优美排列数量等于把 2,3 放在前两位的优美排列数量，因此 `f["100110"] += f["000110"]`。
...
每次滑动 单个 word 的长度，使用哈希表更新当前窗口中的单词个数。当哈希表内存在单词计数高于 words 中的计数时，收缩左边界。

当哈希表内单词数等于 words 中单词数时，添加左边界到答案中。

（可以进一步优化，判断右边界扩充时引入的新单词在不在 words 中，如果不在就直接更新 左边界 = 右边界，并清空当前哈希表）

假设每个单词长度为 w，words 中单词个数为 m，字符串 s 长度为 n。

（一种更朴素的思路是，使用循环每次判断 wm 长度，即子串长度，然后使用截取的子串构建新哈希表并和 words 构建的哈希表比较。但这样复杂度为 O(wmn)）

时间复杂度：构建哈希表 O(m)，滑动窗口中复杂度为 w x (n // w) x w，因此我觉得总复杂度为 O(m + wn)，附加条件是 wm < n
...
`dp[i][j][k]` 表示第 i 天记录上有 j 个 "A"，末尾有连续 k 个 "L" 的情况。

根据第 i 天可能的记录为 "P","A","L" 分三种情况考虑。

可使用快速幂优化时间复杂度，需要先把 j 和 k 两个维度的状态转换为 1 维。写出矩阵后进行快速幂运算，具体写法看代码。

时间复杂度：普通 DP 为 O(n)，使用快速幂为 O(log n)

空间复杂度：普通 DP 为 O(n)，可简化至 O(1)；快速幂写法 O(1)
...
如果 `s[i]` 是 `(`，那么直接跳过。如果是 `)`，那么需要结合 `s[i - 1]` 甚至 `s[i - dp[i - 1] - 1]` 来判断。
...
很容易想到一个朴素的匹配方法：左括号下标进栈，右括号出栈。问题是：用当前的 右括号下标 - 左括号下标 不一定是当前右括号对应的
最长有效括号。

解决方法是，通俗地说：维持栈底元素为 “最后一个没有被匹配的右括号的下标”，首先插入 -1.

对于当前字符：

- 左括号：下标进栈

- 右括号：栈顶元素出栈。如果此时栈空，则说明当前位置没有匹配到，则当前位置成为新的栈底。如果栈不为空，则栈底元素为 “最后一个没有被匹配的右括号的下标”，
此时从栈底（不包括栈底）到当前右括号下标的位置都是有效的，因此 `ans = max(ans, i - stack[-1])`
...
使用两个计数器 left 和 right 分别记录当前遇到的左括号和右括号的数量。

当 left == right，更新结果 `ans = max(ans, 2 * right)`

当 left < right，说明有没有被匹配的右括号，重置 left = right = 0

但由于 `(()` 这一种情况没有办法匹配到，因此从后往前再遍历一次，只需要修改重置的套件为 left > right 即可。
...
通过不断对除数翻倍来达到快速寻找答案的目的。每次翻倍到刚好小于 a 的当前值的时候停止（意思是对除数再翻倍一次就大于 a 了），然后
从 a 中减去得到的数，并将当前基于 b 的倍数加入到答案中（具体参考代码中的 简化代码 部分）
...
右指针移动到满足条件（和大于等于 target）的位置后，开始收缩左指针直至不满足条件。如果一开始是从满足条件开始移动的，则更新最终结果，否则不更新。
...
从 0 到 n - 1 每次使用当前位置 i 作为起点 start，在 `[i, n - 1]` 范围内搜索第一个满足和大于等于 target 的位置。

注意：二分查找的边界判断，如果有等号那么在二分中任何情况都必须缩小边界或者跳出循环。由于我们这里找第一个大于等于 target 的位置，因此最终从返回的
是 left + 1. （注意 left + 1 == n 的特殊情况，此时一定表示不满足条件）
...
假设有 m 条航班信息，n 个航班。暴力解法的复杂度是 O(mn)

使用差分数组。只修改边界部分：左边界加，右边界减。

最后累加前缀和即可得到 answer 数组。
